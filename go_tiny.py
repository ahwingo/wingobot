...
def get_full_state_from_byte_board_history(byte_boards, player_to_go_next):
    """
    Returns a 13x13x9 board state holding the last handful of moves for both players, plus a tensor for who goes next.
...
def get_all_legal_moves_from_board_state(board_state):
    """
    :param board_state: a 13x13x9 board state
    ...
    black_board = board_state[6] if board_state[8][0] == 1 else board_state[7]
    white_board = board_state[6] if board_state[8][0] == 0 else board_state[7]
    ...
    playing_stone_value = 1 if board_state[8][0] == 1 else 2
    for idx in range(169):
...
def update_board_state_for_move(action_idx, board_state):
    ...
    curr_state_curr_player = board_state[6]
    curr_state_next_player = board_state[7]
    prev_state_curr_player = board_state[4]
    prev_state_next_player = board_state[5]
    ...
    for layer in range(2, 8, 2):
        board_state[layer - 2] = board_state[layer + 1]
        board_state[layer - 1] = board_state[layer]

    board_state[6] = new_state_next_player
    board_state[7] = new_state_curr_player
